...
#include <drc/pcb_violation.h>


std::string PCB_VIOLATION::GetTypeAsString( PCB_VIOLATION_TYPE aType )
{
    switch( aType )
    {
    case PCB_VIOLATION_TYPE::BOARD_EDGE:
        return _( "Board edge violations" ).ToStdString();

    case PCB_VIOLATION_TYPE::CLEARANCE:
        return _( "Clearance constraints" ).ToStdString();

    case PCB_VIOLATION_TYPE::COLLISION:
        return _( "Collisions" ).ToStdString();

    case PCB_VIOLATION_TYPE::DANGLING:
        return _( "Dangling items" ).ToStdString();

    case PCB_VIOLATION_TYPE::FOOTPRINT:
        return _( "Footprint violations" ).ToStdString();

    case PCB_VIOLATION_TYPE::KEEPOUT:
        return _( "Keepout constraints" ).ToStdString();

    case PCB_VIOLATION_TYPE::LENGTH:
        return _( "Length constraints" ).ToStdString();

    case PCB_VIOLATION_TYPE::OTHER:
        return _( "Other violations" ).ToStdString();

    case PCB_VIOLATION_TYPE::PADSTACK:
        return _( "Hole/via violations" ).ToStdString();

    case PCB_VIOLATION_TYPE::SHORT:
        return _( "Short circuit violations" ).ToStdString();

    case PCB_VIOLATION_TYPE::TRACK:
        return _( "Track violations" ).ToStdString();

    case PCB_VIOLATION_TYPE::UNCONNECTED:
        return _( "Unconnected items" ).ToStdString();

    case PCB_VIOLATION_TYPE::WIDTH:
